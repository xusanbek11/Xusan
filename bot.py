...
def set_user_vip(user_id, days=30):
    ...
    cursor.execute(
        "INSERT OR REPLACE INTO users (user_id, vip_until) VALUES (?, ?)",
        (user_id, expire_str),
    )
    conn.commit()
    conn.close()
...
DISEASES = {
    "oq doq": {
        ...
        "alomat": (
            "Baliq badanida kichik oq nuqtalar paydo bo'ladi, baliq"
            " obyektlarga ishqalanadi."
        ),
        "davo": (
            "1. Suv haroratini 28-30°C ga ko'taring.\n2. Akvariumga 'Kostapur'"
            " yoki 'Malaxitoviy zelenyy' qo'shing.\n3. Har kuni suvning 25%"
            " qismini tindirilgan suvga almashtiring."
        ),
    },
    "suzogich": {
        ...
        "davo": (
            "1. Suvning 30% qismini almashtiring.\n2. 'Baktopur' yoki"
            " 'Levomitsetin' ishlatiladi."
        ),
    },
    "paxta": {
        ...
        "davo": (
            "1. Baliqni alohida idishga oling.\n2. Tuzli vanna qiling (1L suvga"
            " 1 osh qoshiq tuz, 10 daqiqa)."
        ),
    },
}
...
def check_sub_status(message):
    ...
    if vip_until:
        bot.reply_to(
            message,
            f"✅ Sizda VIP obuna faol!\n📅 Amal qilish muddati:"
            f" {vip_until.strftime('%d.%m.%Y %H:%M')} gacha.",
        )
    else:
...
def create_octo_payment(message):
    ...
    try:
        ...
        if data.get("error") == 0 and "octo_pay_url" in data:
            ...
            cursor = conn.cursor()
            cursor.execute(
                "INSERT INTO payments (trans_id, user_id, status) VALUES (?,"
                " ?, ?)",
                (trans_id, user_id, "pending"),
            )
            conn.commit()
            conn.close()
            ...
            kb.add(
                types.InlineKeyboardButton(
                    "💳 Octo orqali to'lash (30,000 so'm)", url=pay_url
                )
            )
            kb.add(
                types.InlineKeyboardButton(
                    "🔄 To'lovni tekshirish", callback_data=f"check_{trans_id}"
                )
            )

            bot.send_message(
                user_id,
                "VIP obuna sotib olish uchun **To'lash** tugmasini bosing va"
                " to'lov qilgach **'To'lovni tekshirish'** tugmasini bosing:",
                reply_markup=kb,
                parse_mode="Markdown",
            )
        else:
...
def verify_octo_payment(call):
    ...
    try:
        ...
        if status in ["succeeded", "paid"]:
            ...
            bot.send_message(
                user_id,
                "🎉 Tabriklaymiz! Sizning 30 kunlik VIP obunangiz"
...
def ask_disease(message):
    ...
    if not get_user_vip(user_id):
        ...
        kb.add(
            types.InlineKeyboardButton(
                "💳 VIP Obuna sotib olish", callback_data="buy_vip_now"
            )
        )
        bot.send_message(
            message.chat.id,
            "🔒 **Kasalliklarga tashxis qo'yish va davolash yo'riqnomasi faqat VIP"
            " obunachilar uchun!**\n\nTo'liq ma'lumot olish uchun VIP obunani"
            " faollashtiring.",
            reply_markup=kb,
            parse_mode="Markdown",
        )
    ...
    kb.add(
        types.InlineKeyboardButton(
            "⚪ Oq nuqtalar (Manka)", callback_data="dis_oq doq"
        )
    )
    kb.add(
        types.InlineKeyboardButton(
            "✂️ Suzog'ichlar titilishi", callback_data="dis_suzogich"
        )
    )
    kb.add(
        types.InlineKeyboardButton(
            "☁️ Paxtaga o'xshash qoplama", callback_data="dis_paxta"
        )
    )
    bot.send_message(
        message.chat.id, "Baliqda qanday alomat ko'ryapsiz?", reply_markup=kb
    )
...
def show_dis_info(call):
    ...
    if not get_user_vip(user_id):
        bot.answer_callback_query(
            call.id,
            "❌ Davolash yo'riqnomasini ko'rish uchun VIP obuna kerak!",
            show_alert=True,
        )
        return
...
def daily_tip(message):
    bot.send_message(
        message.chat.id,
        "🐠 **Kunlik maslahat:** Baliqlarga yemni kam-kamdan bering. Yem 2"
        " daqiqada yeb tugatilishi kerak. Ortiqcha yem suvni buzadi!",
    )
...
def text_ai_chat(message):
    ...
    if any(w in txt for w in ["salom", "assalom", "privet"]):
        bot.reply_to(
            message,
            "Salom! Baliqlaringiz ahvoli yaxshimi? Bugun yem berdingizmi? 🐠",
        )
    elif "suv" in txt:
        bot.reply_to(
            message,
            "💧 Akvarium suvining 20-30% qismini har hafta almashtirib turing.",
        )
    elif "yem" in txt:
        bot.reply_to(
            message,
            "🍽️ Yemni kuniga 1-2 marta bering, oshiqchasi suvni kirlatadi.",
        )
    else:
        bot.reply_to(
            message,
            "🤖 Men akvarium va baliqlar parvarishi bo'yicha yordamchiman."
